...
class wavenet_mmd(nn.Module):
    def __init__(self, d_lag, d_cov, d_emb, d_output, d_hidden, N, kernel_size, mmd_type='linear', gamma=1.0):
        super(wavenet_mmd, self).__init__()
    ...
    def forward(self, X_emb, X_cov, X_lag, y, d_outputseqlen, domain=None, pretain=False):  
        ...
        if self.training:
            ...
            loss_mmd = self.gamma * self.mmd_loss(source, target)
        else:
            loss_mmd = 0.
